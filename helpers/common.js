...
const validateUrl = url => {
    return /^((https?|ftp|mailto):(\/\/)?)?((([a-z\d]+)(\.|@|:|-))+[a-z]{2,}|((\d{1,3}\.){3}\d{1,3}))(\:\d+)?(\/[a-z\d_\-+~%.()]*)*(\?[a-z\d_\-+~%.,=;&{}]*)?(\#[a-z\d_\-+~%.,=;&{}]*)?$/i.test(url);
}
...
const getHostnameFromReferer = req => {
    return req.headers.referer.match(/^https?\:\/\/([^\/?#]+)(?:[\/?#]|$)/i)[0];
}
module.exports = {
    sendResponse,
    sendResponseMsg,
