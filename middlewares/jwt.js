...
module.exports.validateToken = (req, res, next) => {
    if(!req.headers.authorization) return helper.sendResponse(res,200,{success:false, message:'No token provided'});
    const token = req.headers.authorization.split('Bearer ')[1]
    jwt.verify(token, config.JWT_SECRET, err => {
        if (err) return helper.sendResponse(res,200,{success:false});
